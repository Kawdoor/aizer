      ...
      const { error } = await supabase.from('inventories').insert([
        {
          ...
          parent_space_id: parentType === 'space' ? formData.parent_space_id || null : null,
          parent_inventory_id: parentType === 'inventory' ? formData.parent_inventory_id || null : null,
        },
      ]);
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-zinc-800 sticky top-0 bg-zinc-900">
          <h2 className="text-lg font-light tracking-wider text-white">CREATE INVENTORY</h2>
          <button
            onClick={onClose}
            ...
            <div className="flex border border-zinc-800">
              <button
                ...
                className={`flex-1 py-3 text-sm font-light tracking-wider transition-colors ${
                  parentType === 'space' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'
                }`}
              >
                SPACE
              </button>
              <button
                ...
                className={`flex-1 py-3 text-sm font-light tracking-wider transition-colors ${
                  parentType === 'inventory' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'
                }`}
              >
                INVENTORY
              </button>
